...
vector<char> sum_vects(vector<char> a, vector<char> b) {
    int64_t size = max(a.size(), b.size());
    a.resize(size);
    b.resize(size);
...
vector<char> sub_vects(vector<char> a, vector<char> b) {
    int64_t size = max(a.size(), b.size());
    a.resize(size);
    b.resize(size);
...
bool comp_vects(vector<char> a, vector<char> b, bool gt) {
    if (a.size() != b.size()) {
        if (gt) {
...
class Bigint {
    public:
        ...
        Bigint(vector<char> input, bool i_negative) {
            if (input.size() > 1) {
                auto erase_cnt = 0;
        ...
        friend ostream& operator<<(ostream& out, const Bigint& b) {
            ...
            for (vector<char>::reverse_iterator it = raw.rbegin(); it != raw.rend(); ++it) {
                out << to_string(*it);
            }
            return out;
        }
     ...
     Bigint operator+(Bigint b) {
        ...
        Bigint operator*(Bigint b) {
            auto b_raw = b.get_raw();
            char carry = 0;
            vector<char> res(num.size() + b_raw.size() + 1);
